...
##### Instrument :
`The term used to describe financial instruments, instruments on foreign exciange (FX) and commodities.`
...
##### Risk position: 
`The portion of the current value of an instrument that may be subject to losses dueto movements in a risk factor. For example, a bond denominated in a currency different to a bank's reporting currency has risk positions in general interest rate risk, credit spread risk (non securitisation) and FX risk, where the risk positions are the potential losses to the current value of the instrument that could occur due to a change in the relevant underlying risk factors (interes rates, credit spreads, or exchange rates).`
...
##### Delta risk: 
`The linear estimate of the change in valiue of a financial instrument due to a movementin the value of a risk factor. The risk factor could be the price of an eguity or commodity, or a change in an interest rate, credit spread or FX rate.`

##### Vega risk: 
`The potential loss resulting from the charnge in value of a derivative due to a change in the implied volatility of its underlying.`

##### Curvature risk: 
`The additional potential loss beyond delta risk due to a change in a risk factor for financial instruments with optionality. In the standardised approach in the market risk framework tis based on two stress scenarios involving an upward shock and a downward shock to each regulatory risk factor.`

##### Value at risk VaR : 
`A measure of the worst expected loss on a portfolio of instruments resultingTrom market movements over a given time horizon and a pre-defined confidence level.`

##### Expected shortiall (ES): 
`A measure of the average of all potential losses exceeding the VaR atagiven confidence level.`

##### Jump-to-defaut UTD) : 
`The risk of a sudden default. JTD exposure refers to the loss that could beincurred rom a JTD event.`
...
###### Diversification: 
`The reduction in risk at a portfolio level due to holding risk positions in differentinstruments that are not perfectly correlated with one another.`
...
`A method for assessing the robustness of banks riskmanagement models by comparing the risk-theoretical PBL predicted by trading desk risk management models with the hypothetical P&L.`

##### Trading desk risk management model: 
`The trading desk risk management model (pertaining to in-scope desks) includes all risk factors that are included in the bank's ES model with supervisoryparameters and any risk factors deemed not modellable, which are therefore not inctuded inES model for calculating the respective regulatory capital requirement, but are included inNMRFS.`

##### Acual P&L (APL): 
`The actual P&L derived from the daily P&L process. It indludes intraday tradingas wel as time effects and new and modified deals, but excludes fees and commission5 as wel a5 valuation adjustments for which separate regulatory capital approaches have been otherwise specified as part of the rules or which are deducted from Common Equity Tier 1. Any other valuation adjustments that are market risk-related must be included in the APL As is the case for the hypothetical P8L, the APL should include FX and commodity risks from positions held in the banking book.`
